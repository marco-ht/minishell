...
/*   Updated: 2025/07/22 16:06:22 by mpierant &       ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		int *p_last_exit_status, t_vars *v)
{
	...
	{
		setup_signals_child();
		redir_stdout = apply_redirs(ctx->pcmd->left, p_last_exit_status, v);
		if (redir_stdout != 1)
			dup2(ctx->p[1], 1);
		close(ctx->p[0]);
		close(ctx->p[1]);
		...
		exit_status = ft_runcmd(ctx->pcmd->left, envp, p_last_exit_status, v);
		free_tree(v->tree);
		ft_free_envp(*envp);
		exit(exit_status);
	}
		...
		int *p_last_exit_status, t_vars *v)
{
	...
	{
		setup_signals_child();
		redir_stdin = apply_redirs(ctx->pcmd->right, p_last_exit_status, v);
		if (redir_stdin != 2)
			dup2(ctx->p[0], 0);
		close(ctx->p[0]);
		close(ctx->p[1]);
		...
		exit_status = ft_runcmd(ctx->pcmd->right, envp, p_last_exit_status, v);
		free_tree(v->tree);
		ft_free_envp(*envp);
		exit(exit_status);
	}
...
int	run_pipe_cmd(t_cmd *cmd, char ***envp, int *p_last_exit_status, t_vars *v)
{
	t_pipe_ctx	ctx;
	...
	run_pipe_left(&ctx, envp, p_last_exit_status, v);
	run_pipe_right(&ctx, envp, p_last_exit_status, v);
	close(ctx.p[0]);
	close(ctx.p[1]);
